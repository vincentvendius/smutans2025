...
generate_gubbins_ggtree <- function(gubbins_tree, clades = NA, branch_width = NA, show_taxa = FALSE, bl_threshold = Inf, label_size = 4, tree_axis_expand = 5, legend_direction = NA) {
  highlighted<-c("NEO105","NEO137","VK63","CGG101233")
  # Read in tree
  gubbins_ggtree <- 
...
#  corrected_length <- tree +
 #   geom_tree() +
  ...
  if (show_taxa) {
    gubbins_ggtree <-
      gubbins_ggtree + 
      geom_tiplab(size = label_size)#aes(subset = label %in% c("NEO105","NEO137","VK63","CGG101233")),
  }
...
make_individual_heatmap <- function(i, df = meta.df, tree_plot = gubbins_tree, legend_direction = "horizontal", ncol = 1, label_size = NA) {
  ...
  if(title_text=="Cluster"){
    palettecustom<- scale_colour_manual(values=c("Cluster_A_ancient"="black","Cluster_A"="#ffce54",
                                 "Cluster_B"="#ac92eb","Cluster_C"="#4fc1e8","Cluster_D"="#a0d568",
                                 "Cluster_D_ancient"="black","Cluster_E"="#ed5564"),
                        aesthetics = c("fill","colour"),
                        name="Chromopainter \nCluster")
  }else if(title_text=="Origin"){
    palettecustom<-scale_colour_manual(values=c("Denmark"="#fc7d0b","United Kingdom"="#c85200","USA"="#5fa2ce",
                               "Brazil"="#1170aa","China"="#50A14F","Other"="#57606c"),na.value="#FFFFFF",
                        aesthetics = c("fill","colour"),
                        name="Origin",na.translate = F)
  }else if(title_text=="Serotype"){
    palettecustom<-scale_colour_manual(values=c("C"="#1170aa","E"="#c85200","F"="#c8d0d9",
                               "K"="#ffbc79"),na.value="#FFFFFF",
                        aesthetics = c("fill","colour"),
                        name="Serotype",na.translate = F)
  }else if(title_text=="Source"){
    palettecustom<-scale_colour_manual(values=c("Ancient teeth"="#f0bd27","plaque"="#ffda66","oral cavity"="#e39802",
                               "stool"="#51b364","blood"="#b60a1c","Other"="#57606c"),na.value="#FFFFFF",
                      aesthetics = c("fill","colour"),
                      name="Source",na.translate = F)
  }else{
    palettecustom<- scale_colour_manual(values = individual_palette,
                        aesthetics = c("fill","colour"),
                        name = title_text) 
  }
