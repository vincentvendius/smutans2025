...
bamPath <- args[5]
outPlot <- args[6]
ani_rep <- read.table(args[7],quote="\"", comment.char="")
outBestmap <- args[8]
out_tsv <- args[9]
## --------------------------------------------------------------------------------
## helpers
...
f1 <- list.files(path = paste(bamPath,"bam/",sampleId, sep = ""),
                 pattern = "genomecov",
                 full.names = TRUE)
...
f1 <- list.files(path = paste(bamPath,"bam/", sampleId, sep = ""), pattern = "filter.bam$", full.names = TRUE)
#f1 <- list.files(path = paste(bamPath,"/bam", sampleId, sep = ""), pattern = "filtered.bam$", full.names = TRUE)
